...
#define MAIN_DATABASE_NAME @"AddressBook.db"

@implementation DBConnection

+ (sqlite3*)openDatabase:(NSString*)dbFilename
{
    sqlite3 * instance = nil;
	BOOL success       = NO;
	NSError * error;
	...
	if(!success)
	{
		NSString *defaultDBPath = [[[NSBundle mainBundle] bundlePath] stringByAppendingPathComponent:MAIN_DATABASE_NAME];
		NSLog(@"%@",defaultDBPath);
    ...
    if (success && sqlite3_open([path UTF8String], &instance) != SQLITE_OK) 
	{
...
+ (void)alert
{
	...
	NSLog(@"Local cache db error[%@]",sqlite3err);
	/*
    [[LeisurelinessTDAppDelegate getAppDelegate] alert:@"Local cache db error" message:sqlite3err];
	*/
}
